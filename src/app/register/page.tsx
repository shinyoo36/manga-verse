                ...
                <div className="w-full h-full max-w-[600px] flex flex-col items-center px-4 gap-y-12 mb-5">
                    ...
                    <div className="bg-gray-700 bg-opacity-35 px-5 py-6 rounded-md min-h-[250px] w-full flex flex-col items-center">
                        <div className="w-full flex flex-col items-center gap-y-3 pb-3">
                            <p className="text-[18px] font-semibold mb-2">Register</p>
                            <GoogleLoginButton/>
                        ...
                        <div className="w-full flex flex-col gap-y-4">
                            ...
                            <button onClick={() => signIn('default')}>
                                <p className='w-full bgOren text-center py-2 rounded-sm text-[14px] font-medium' >Register</p>
                            </button>
                            {errorMessage && (
                            ...
                            <div className="flex flex-col gap-y-2">
                                <div className="flex flex-row gap-x-2 w-full items-center justify-center">
                                    <p className="text-[14px] font-regular ">Already have an account ?</p>
                                    <Link href={"/sign-in"} passHref>
                                        <p className="text-[14px] textOren font-regular">Sign In</p>
                                    </Link>
                                </div>
...
const GoogleLoginButton = () => {
    ...
    const login = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            if(userInfo) {
                register(userInfo.email, userInfo.name, 'google');
            }
        },
        onError: () => console.log("Login Failed"),
    });

    const register = async (email: string, name: string, provider: string) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/users`, {
                method: "POST",
                headers: {
